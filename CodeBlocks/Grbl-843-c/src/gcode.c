...
// void gc_set_current_position(int32_t x, int32_t y, int32_t z, int32_t u)
{
  ...
  // gc.position[U_AXIS] = u/settings.steps_per_mm[U_AXIS];
/// axis a : mm = degree  !!
...
/// TODO : 842 : to_millimeters ==> to_degrees
        target[A_AXIS] = to_degrees(value);
        bit_true(axis_words,bit(A_AXIS));
        break;
       /*
      case 'U':
        target[U_AXIS] = to_millimeters(value);
  ...
  switch (non_modal_action) {
    ...
    case NON_MODAL_GO_HOME:
     ...
     /// -> 843 : 4 -> N_AXIS
        ...
        /// TODO : axis U
        // mc_line(target[X_AXIS], target[Y_AXIS], target[Z_AXIS], target[U_AXIS], settings.default_seek_rate, false);
      }
      mc_go_home();
    ...
    switch (gc.motion_mode) {
      ...
      case MOTION_MODE_SEEK:
        if (!axis_words) {
            ...
            mc_line(target[X_AXIS], target[Y_AXIS], target[Z_AXIS], target[A_AXIS],
                settings.default_seek_rate, false);
            /*  TODO
			mc_line(target[X_AXIS], target[Y_AXIS], target[Z_AXIS], target[U_AXIS],
                settings.default_seek_rate, false);
			*/
        }
        break;
