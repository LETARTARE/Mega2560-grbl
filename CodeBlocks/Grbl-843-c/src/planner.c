...
// TODO
  // target[U_AXIS] = lround(z*settings.steps_per_mm[U_AXIS]);
/// mm = degree
...
// TODO
  // if (target[U_AXIS] < pl.position[U_AXIS]) { block->direction_bits |= (1<<U_DIRECTION_BIT); }
...
// TODO
  // block->steps_u = labs(target[U_AXIS]-pl.position[U_AXIS]);
...
// TODO
  // delta_mm[U_AXIS] = (target[U_AXIS]-pl.position[U_AXIS])/settings.steps_per_mm[U_AXIS];
/// mm = degree
  ...
  /*  TODO
  block->millimeters = sqrt(delta_mm[X_AXIS]*delta_mm[X_AXIS] + delta_mm[Y_AXIS]*delta_mm[Y_AXIS] +
                            delta_mm[Z_AXIS]*delta_mm[Z_AXIS] + delta_mm[A_AXIS]*delta_mm[U_AXIS]);
  */
...
// TODO
  // unit_vec[U_AXIS] = delta_mm[U_AXIS]*inverse_millimeters;
  ...
  if ((block_buffer_head != block_buffer_tail) && (pl.previous_nominal_speed > 0.0)) {
    ...
    double cos_theta = - pl.previous_unit_vec[X_AXIS] * unit_vec[X_AXIS]
                     ...
                     // TODO
                       // - pl.previous_unit_vec[U_AXIS] * unit_vec[U_AXIS]
...
// TODO
// void plan_set_current_position(int32_t x, int32_t y, int32_t z, int32_t u)
{
  pl.position[X_AXIS] = x;
  pl.position[Y_AXIS] = y;
  pl.position[Z_AXIS] = z;
  // pl.position[U_AXIS] = u;
