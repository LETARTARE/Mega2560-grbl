...
// TODO
// static void homing_cycle(bool x_axis, bool y_axis, bool z_axis, bool u_axis, int8_t pos_dir, double homing_rate)
{
...
/// -> 843 : 4 -> N_AXIS
  ...
  */
  if (a_axis)  {
///  axis a : mm = degree
	steps[A_AXIS] = lround(settings.steps_per_mm[A_AXIS]);
  }
...
// 843 TODO
//double ds = step_event_count/sqrt(x_axis + y_axis + z_axis + u_axis);
...
/* TODO
	if (u_axis) {
      ...
      if (counter_u > 0) {
        if (LIMIT_PIN & (1<<U_LIMIT_BIT)) { out_bits ^= (1<<U_STEP_BIT); }
        else { u_axis = false; }
...
*/
  ...
  /// 842 : error counter_x -> counter_a
    ...
    // TODO
	//if (!(x_axis || y_axis || z_axis || u_axis) || sys.abort) { return; }
    // Perform step.
